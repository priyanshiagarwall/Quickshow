      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex flex-wrap gap-6 mt-4 w-max'>
          {nowPlayingMovies.map((movie) => (
            ...
            >
              {/* Poster + Rating/Votes + Selected check */}
              <div className='relative'>
                <img
                  src={movie.poster_path}
                  alt={movie.title}
                  className='w-full h-[240px] object-cover brightness-90 rounded-md'
                />
                <div className='text-sm flex items-center justify-between px-2 py-1 bg-black/70 w-full absolute bottom-0 left-0'>
              ...
              {/* Title and Release Date below the poster */}
              <div className='mt-2'>
                ...
                {Object.entries(dateTimeSelection).map(([date, times]) => (
                  <li key={date}>
                    ...
                    <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                      {times.map((time) => (
                        <div key={time} className='border border-primary px-1 py-1 flex items-center rounded'>
                          <span>{time}</span>
